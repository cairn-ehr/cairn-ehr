...
- **Raised, not fixed — four.** [#467](https://github.com/cairn-ehr/cairn-ehr/issues/467): the required CI floor job flaked once and could say only *"loading 031_medication: db error"*, because `crates/cairn-node/src/db.rs` has NINE `anyhow!("…: {e}")` wrappings over `tokio_postgres::Error` — #109's species in the file that fails first and can least explain itself, so the flake stays undiagnosable until the message exists. [#468](https://github.com/cairn-ehr/cairn-ehr/issues/468): **this alert fires once ever**, while `custody_withheld` — the precedent it claims to follow *exactly* — re-fires every cycle the condition stands; and unlike an unfinished peering ceremony, an unlearnable reference is unrepairable in place, so the steady state is a node permanently holding unfetchable clinical attachments with no recurring signal. A judgement about what an operator should be woken for, not a patch. [#469](https://github.com/cairn-ehr/cairn-ehr/issues/469): a failed `sync_state` cursor commit is the one propagation point between the apply loop and the metrics build, so it loses the whole metrics object — and `run` classifies anything that is not a `PullIntegrityError` as `"partition": true`, blaming the WAN for a local DB fault. [#470](https://github.com/cairn-ehr/cairn-ehr/issues/470): the per-cycle ledger read is the only production read of db/050's table that bypasses its `SECURITY DEFINER`, and works only because cairn-sync already needs owner privileges (`sync_state` carries no grants at all); recorded in db/050's header so a future move onto the unprivileged runtime role does not discover it by failing every cycle.
