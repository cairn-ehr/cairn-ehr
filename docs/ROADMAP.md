...
- **Residual: [#461](https://github.com/cairn-ehr/cairn-ehr/issues/461)** — the rule has three implementations (ADR-0058's `clock_grade`/db/040 · ADR-0063's `safety` · db/050) and no ADR naming it, so it is findable only under another field's title. Documentation-only; all three already comply.
...
wrong cluster and `dropdb`-ing before the #169 marker check). #458 #461 — attachment shape is unvalidated, and the mint-strict/arrive-permissive rule needs a name:
a non-object attachment element is admitted, replicated, and has no legible descriptor. #392 #393 — federation
