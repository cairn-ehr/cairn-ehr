...
- **Both admit paths, not just the one the issue named:** `pull` and `requeue` reach the same db/020 door, so both report `references_unlearnable` — requeue being the easier to miss, since it prints one cheerful "released" line per row. **The report is keyed on TWO things, and reviewing PR #466 is what found the second.** The addresses of every event the door admitted keep another peer's concurrent defect from being charged to this one. A **`flag_id` watermark** taken before the run is what makes it this run's *news* — and v1, which keyed on newly-INSERTED events instead, shipped a **silent zero**: db/020 calls `cairn_learn_attachment_refs_lenient` unconditionally (after its `ON CONFLICT DO NOTHING`, no `v_rows` guard), so the learner re-runs on an idempotent re-apply and can write a first-ever flag row for an event already held. A node upgrading onto db/050 flags its whole pre-#460 back-catalogue on the next full sweep and would have reported `0` for all of it — the exact defect class #465 exists to end, on the cycle carrying the most news. Dropping either clause now turns five tests red. **One alert, not one per cycle forever**, which is how an operator learns to ignore a line; the standing picture is a different question with a different surface. **And a failed ledger read reports `null`, never `0`** — zero is a claim ("I looked, there were none"), which after a failed read would mute a monitor alerting on `> 0` exactly when this node has stopped being able to see (principle 4). **And that "never silent" line said `db error`** — `postgres::Error`'s `Display` is a bare match on kind with no source chaining, so `.map_err(|e| e.to_string())` rendered a missing db/050, a revoked grant and a statement timeout identically: three remedies, one indistinguishable line, on the arm whose whole justification is that a failed read is never silent. `legible_db_error` now carries message + SQLSTATE + DETAIL. **#467's species, committed on #467's own branch**, with `ApplyError::from` twelve hundred lines up already showing the fix.
- **`cairn_attachment_flag_health()` had no caller — the Slice 69 finding on a new surface.** New verb `cairn-sync attachment-flags` prints it, one JSON row per (event type, reason), each NAMING an example event, plus a total (a group count understates by an unbounded factor). **Peer text is not display text — twice.** The ledger `reason` embeds a bounded prefix of the peer's own value (db/001's hex accessor caps it at 8 characters so a short secret is never reproduced whole; what separates a truncated digest from a wrongly-encoded one is the DETAIL beside it, not the prefix), so a newline in it forged a whole log line, escaped with `{:?}`. The review then found the **larger** channel on the very line cited as this one's precedent: `custody_withheld` is unbounded prose from an *unadmitted* peer, printed raw — enough to forge a `0 attachment reference(s)` all-clear on the alarm #465 had just installed. Extracted to `custody_withheld_message` and escaped. **The operator line is also now written by the ONLY producer of the metric**, through a `&mut dyn Write` sink, so a caller cannot keep the count and drop the line and the write itself is testable — it was an `eprintln!` no test could see.
- **Raised — four; two closed the same day.** [#467](https://github.com/cairn-ehr/cairn-ehr/issues/467) (**closed, see below**): the required CI floor job flaked once and could say only *"loading 031_medication: db error"*. [#468](https://github.com/cairn-ehr/cairn-ehr/issues/468): **this alert fires once ever**, while `custody_withheld` — the precedent it claims to follow *exactly* — re-fires every cycle the condition stands; and unlike an unfinished peering ceremony, an unlearnable reference is unrepairable in place, so the steady state is a node permanently holding unfetchable clinical attachments with no recurring signal. A judgement about what an operator should be woken for, not a patch. [#469](https://github.com/cairn-ehr/cairn-ehr/issues/469) (**closed, see below**): a failed `sync_state` cursor commit lost the whole metrics object and was logged as a partition. [#470](https://github.com/cairn-ehr/cairn-ehr/issues/470): the per-cycle ledger read is the only production read of db/050's table that bypasses its `SECURITY DEFINER`, and works only because cairn-sync already needs owner privileges (`sync_state` carries no grants at all); recorded in db/050's header so a future move onto the unprivileged runtime role does not discover it by failing every cycle.

**The two places a database failure said nothing (2026-08-22; closes [#467](https://github.com/cairn-ehr/cairn-ehr/issues/467), [#469](https://github.com/cairn-ehr/cairn-ehr/issues/469); opens [#471](https://github.com/cairn-ehr/cairn-ehr/issues/471), and from its own review [#473](https://github.com/cairn-ehr/cairn-ehr/issues/473), [#474](https://github.com/cairn-ehr/cairn-ehr/issues/474), [#475](https://github.com/cairn-ehr/cairn-ehr/issues/475), [#476](https://github.com/cairn-ehr/cairn-ehr/issues/476); `crates/cairn-node` + `crates/cairn-sync` + `bet_a.py` — no migration file, no ADR, SCHEMA stays 50).** Both are #109's species and both were found by a real failure — and the review of the fix then found a third in the fix itself (below).

- **#467 — the wrapper that added context subtracted the diagnosis.** `tokio_postgres::Error`'s `Display` is the literal string `"db error"`: a bare match on kind that never chains to the source holding the message, the DETAIL and the **SQLSTATE**. `42P01` (schema never loaded here), `42501` (a missing grant), `40P01`, `53300`, `57014` are five different operator actions and one indistinguishable line — and `anyhow!("…: {e}")` *also discards the source*, so anyhow's own chain printing had nothing left to show either. That is the whole content of the CI line `loading 031_medication: db error`. New module `cairn-node::db_diagnosis`: a **pure** `compose_db_diagnosis(message, sqlstate, detail)` (unit-testable with no database — `DbError` cannot be constructed by hand, but its *rendering* can) plus a thin `legible_db_error` extractor falling back to `Display` for a dropped connection or an unparseable connection string, where the kind IS the whole story. The composed shape is **byte-identical to `cairn-sync`'s renderer** so an operator grepping a node log and a sync log learns one format; the duplication is deliberate (cairn-sync does not depend on cairn-node, and a new workspace crate for twenty lines costs more than it saves) and each doc comment names the other. All 9 `anyhow!` sites and all 5 bare `?` sites in `db.rs` route through it, the bare ones gaining the context they never had, and the `connect` door **never echoes the connection string** — it can carry a password. **A three-line loop body cannot be tested**, and #467's acceptance criterion is a sentence about a message, so the replay loop's body is now `db::load_migration`, a named public door drivable with a deliberately failing body; the test asserts the migration name, the `[42883]` and the server's own text. Also corrected here: HANDOVER said the test guard "self-serializes cluster-wide", but **PostgreSQL advisory locks are scoped per database** — which is exactly why `db::test_serial_guard`'s own comment already requires every caller to lock against `CAIRN_TEST_PG` specifically. Whether that per-database scoping is behind the `clinical_pull` flake is still unknown; #467 was the prerequisite, not the diagnosis.
- **#469 — a local write failure logged as link downtime.** `do_pull`'s cursor commit is the ONLY propagation point between the apply loop and the metrics object (verified: every earlier return either precedes any apply or is routed deliberately; the two `?` inside the decode closure are captured into `decoded`, not propagated), so a bare `?` there lost `applied_new`, `custody_withheld`, `references_unlearnable`, every number describing work that had ALREADY happened and was durable — and `run`'s catch-all then wrote `"partition": true`, sending an operator to the WAN for a local database fault and charging the Bet A availability figure for it. `PullIntegrityError` means *the peer answered and its DATA is the problem*; `partition` means *the link*; neither fits, so folding it into either just states a different wrong diagnosis. New third class `CursorCommitError` → a `"local_fault"` log key, with the metrics built BEFORE the commit and the mapping in a pure `classify_pull_failure` whose default arm is `partition` **last** — an unclaimed failure is, by elimination, the one where nothing was reached. **And a failed write must not report the values it meant to set:** `cursor_seq` and `floor_active` are claims *about that write*, so publishing them afterwards tells a monitor the cursor advanced when it did not — and after a mid-statement connection loss this node genuinely cannot know. Both go `null` (`mark_cursor_outcome_unknown`), `references_unlearnable`'s null-never-zero rule one field over, with the attempted seq named in the message instead. The #465 ledger read still runs **after** the commit attempt, including when it failed: the events and flags are durable regardless, so it remains a true report about completed work. **A key nothing counts is the silence the fix exists to end** — `bet_a.py` now sums and prints `local_fault` beside `partition`/`integrity`.
- **Test mechanic worth reusing.** To force a write failure inside a SHARED test database, hold a `FOR UPDATE` row lock from a second connection while the code under test runs under a short `lock_timeout`. A trigger or a `REVOKE` persists if the test panics and poisons every later suite; a row lock is released when its connection drops.
- **Raised, not fixed: [#471](https://github.com/cairn-ehr/cairn-ehr/issues/471).** `do_requeue` carries the same bare `?` sites — **three**, not two — *inside* its loop, after events have been released through the apply door — so a failure on row 5 of 20 loses the whole report of what the first four achieved and says `"db error"`. **ADR-0060 decision 2** (partial completion must be reported, never implied) is the argument; #469's fix is the worked example. Smaller than #469 in one respect: `do_requeue` is a one-shot CLI verb, not a cycle in `run`, so no misclassification follows.
- **The review of that PR found a defect at its own centre, and it was fixed on the same branch.** `legible_db_error`'s fallback arm was **#467 one kind over**: `tokio_postgres::Error`'s `Display` is a bare kind match for EVERY kind, not just `Kind::Db`, and never chains to `source()` — so `error connecting to server` was the whole of what the node said about a refused socket, an unresolvable host and a TLS timeout alike. It was also a **regression**: `db::connect` had been a bare `?`, which anyhow preserved, so `main` printed `Caused by: Connection refused (os error 61)` until the new wrapper built its error from a `String` and deleted it. **And `hint()` was dropped at all fourteen sites** — `DbError`'s own `Display` prints message + DETAIL + **HINT**, so on the `42883` the schema loader hits, PostgreSQL's own remedy was thrown away. The arm now walks the source chain, the composer carries HINT, and — the reason it shipped at all — **the test covering that arm could not fail**: it asserted only `!= "db error"` and non-empty, both true of the broken output. Three further findings were real defects rather than polish: **#469's misdiagnosis survived two statements above the fix** (`do_pull`'s first two statements are bare `?` on a `postgres::Error` *before any network I/O*, and `cmd_run` builds its client once outside the loop — so after the database went away, every cycle for the life of the process was logged as link downtime; since the peer is reached over a raw `TcpStream`, any postgres error escaping `do_pull` is by construction local, which is now an explicit arm); **the operator line asserted what the metric withheld** ("the cursor did not advance" beside a `cursor_seq` nulled *because* this node cannot know); and **a failed commit swallowed a co-occurring integrity condition**, taking `ack-quarantine`/`requeue`/re-sign with it while telling the operator the events "re-apply idempotently" — which a quarantined event does not. A new source-scan guard (`db_errors_stay_legible.rs`) now fails on any `{e}` left in `db.rs`, aimed not at the fourteen sites that exist but at the fifteenth. Opens **[#473](https://github.com/cairn-ehr/cairn-ehr/issues/473)** (the same species on a **clinical safety surface** — `safety.rs` logs `db error` from the very line written to make a degraded safety projection distinguishable from a correctly empty one), **[#474](https://github.com/cairn-ehr/cairn-ehr/issues/474)** (`cairn-node`’s pull loop: the deny-all reason, the FREEZE reason, a `PARTITION` asserted over an unread error, and `let _ = connection.await;`), **[#475](https://github.com/cairn-ehr/cairn-ehr/issues/475)** (`cairn-sync init` still cannot name the migration that failed), **[#476](https://github.com/cairn-ehr/cairn-ehr/issues/476)** (~120 comments still call a per-database advisory lock "cluster-wide").
