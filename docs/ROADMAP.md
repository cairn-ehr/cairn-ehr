...
**The two places a database failure said nothing (2026-08-22; closes [#467](https://github.com/cairn-ehr/cairn-ehr/issues/467), [#469](https://github.com/cairn-ehr/cairn-ehr/issues/469); opens [#471](https://github.com/cairn-ehr/cairn-ehr/issues/471); `crates/cairn-node` + `crates/cairn-sync` + `bet_a.py` — no migration file, no ADR, SCHEMA stays 50).** Both are #109's species and both were found by a real failure. Full local gate green: **1463 tests / 133 binaries / 0 failed**, SQL mirrors PASS.
...
- **Raised, not fixed: [#471](https://github.com/cairn-ehr/cairn-ehr/issues/471).** `do_requeue` carries the same two bare `?` sites *inside* its loop, after events have been released through the apply door — so a failure on row 5 of 20 loses the whole report of what the first four achieved and says `"db error"`. **ADR-0060 decision 2** (partial completion must be reported, never implied) is the argument; #469's fix is the worked example. Smaller than #469 in one respect: `do_requeue` is a one-shot CLI verb, not a cycle in `run`, so no misclassification follows.
