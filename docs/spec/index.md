...
**Status:** Architecture spec essentially complete (all open questions resolved) and viability proven by proof-of-concept spikes (walking skeleton, WAN sync, a first federating node, Postgres-on-Android). The **first production clinical surface is now under construction** — the demographics subsystem on `cairn-node`, with several slices landed (§4.4 identifiers, §4.2 DOB + sex-at-birth, §4.2 names, §4.2 administrative-sex + gender-identity, §4.3 address).
**Spec version:** 0.49 · **License target:** AGPL-3.0 (all components AGPL-3.0-compatible).
