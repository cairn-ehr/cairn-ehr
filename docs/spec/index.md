...
**Status:** Architecture spec essentially complete (all open questions resolved) and viability proven by proof-of-concept spikes (walking skeleton, WAN sync, a first federating node, Postgres-on-Android). The **first production clinical surface is under construction** on `cairn-node`: the demographics subsystem (§4.4 identifiers, §4.2 DOB/sex-at-birth/names/administrative-sex/gender-identity, §4.3 address), the advisory §5.2 matcher, the §5.7 identity core, the §5.4 John-Doe subsystem and the §5.3/§5.8 search-before-create funnel (registration as an act carrying its search — ADR-0061; its precedence rule **is enforced** at the strict door, so the first event on a chart must be its registration), the first clinical-content stream `clinical.medication` (§3.3, five slices: assert/cease, dose timeline, reconciliation, attestation, per-field correction) under born-sealed bodies (§3.5/ADR-0052) with per-write human authorship (§3.9/ADR-0053), the §5.9 sensitivity stream (graded append-only confidentiality assertions and their effective-grade projection — ADR-0062; it computes and reports a grade and **still withholds no content**, because custody narrowing is decided but not yet built — ADR-0065 settles it as a **custody ladder** whose invariant is that narrowing changes the cost and the noise of reading, never whether the content can be reached at a node that holds the key or can reach one, since an audited break-glass is available at every rung — bounded at rung 1, whose glass is a network act, so a partitioned non-holder still cannot reach it (#498) — but since ADR-0064 only an **authoritative** withdrawal lowers it: a vouched attestation by an enrolled human, or the human withdrawing their own claim, with everything else admitted, inert and listed on a worklist — *admit the claim, withhold the power*) with its safety projection now emitted beside a sealed body whose coding this deployment's class map recognises (a de-identified class + severity coarsened down a three-rung ladder by that grade — ADR-0063; the seal boundary is the coarsening boundary, so a custody-less or crypto-shredded reader still gets a signal that names nothing — and the map **ships empty**, because Cairn ships the lookup and never the drug knowledge, so a stock node emits none until its deployment populates it), and the first slice of the reference-UI shell (Tauri 2, an L3 choice below the §9.5 compatibility boundary).
**Spec version:** 0.68 · **License target:** AGPL-3.0 (all components AGPL-3.0-compatible).
