...
| [0060](0060-partial-validity-a-defect-on-one-line-never-invalidates-another.md) | Partial validity: a defect on one element of a composite clinical object never invalidates the others — never refuse the whole for a defect in a part; partial completion must be REPORTED, never implied (the line stays visibly unsigned, paper's blank signature box); uncertainty may only widen the account of what was not done; the exclusion must name what would repair it; the sole admissible whole-operation refusal is "is this the same object the human reviewed?", never "is it perfect?"; and transaction scope must match clinical atomicity — independently-actionable lines never share a transaction, so a rollback does no collateral damage | Accepted (corollary of principle 3 / §1.2; applies principle 4 + 0047/0049) | 2026-08-03 |
