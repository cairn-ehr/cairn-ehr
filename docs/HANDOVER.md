...
- **Exit criteria met → two ADRs now unblocked** (write them next, citing the spike): (1) the parked **ADR-0011 skill-epoch refinement**; (2) an **advisory-actor integration-contract ADR** promoting [ecosystem/0001](ecosystem/0001-agent-and-messaging-plugins-kastellan-localmail.md) from *evaluation* to *decision*.
...
- **Still build-prep; the remaining threads are unchanged:** the **Bet B Pi compute-cost run** (still awaiting the board) and continued **clinical case-mining**. The two unblocked ADRs are the natural next generative step.
