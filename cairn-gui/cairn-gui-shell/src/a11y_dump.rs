...
use cairn_gui_tab::{
    Capabilities, Context, Field, PatientRef, Role, Semantic, SemanticNode, UserRef,
};
...
fn shell_chrome(patient_name: &str) -> SemanticNode {
    SemanticNode {
        ...
        fields: vec![
            ...
            Field {
                id: "chrome.identity".into(),
                role: Role::Heading,
                label: format!("Patient: {}", patient_name),
            },
            ...
            Field {
                id: "chrome.pane.left".into(),
                role: Role::Pane,
                label: "Left pane".into(),
            },
            Field {
                id: "chrome.pane.right".into(),
                role: Role::Pane,
                label: "Right pane".into(),
            },
            // Labels match `title()` as rendered by app.rs's tab-strip buttons.
            Field {
                id: "chrome.tab.left.note".into(),
                role: Role::Tab,
                label: "Current note".into(),
            },
            Field {
                id: "chrome.tab.right.demographics".into(),
                role: Role::Tab,
                label: "Demographics".into(),
            },
            Field {
                id: "chrome.divider".into(),
                role: Role::Divider,
...
mod tests {
    ...
    fn expected_tree_covers_shell_chrome_and_is_complete() {
        ...
        let chrome = nodes
            .iter()
            .find(|n| n.title == "Shell chrome")
            .expect("chrome node present");
        assert!(
            chrome.fields.iter().any(|f| f.role == Role::Divider),
            "divider must be in the a11y tree (keyboard-resizable)"
        );
        assert!(
            chrome.fields.iter().any(|f| f.role == Role::Tab),
            "tab strip stops must be in the a11y tree"
        );
        // ...and every node must pass the completeness contract.
        for n in &nodes {
            n.assert_complete()
                .unwrap_or_else(|e| panic!("incomplete node {}: {e}", n.title));
        }
    }
}
