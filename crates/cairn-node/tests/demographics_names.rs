...
async fn happy_path_and_retained_set() {
    ...
    for (val, use_, prov, wall) in [
        ...
        submit_field(
            ...
            Some(&render_name_twin(val, Some(use_), prov)),
        )
        .await
        .unwrap();
    }
    assert_eq!(
        ...
        3,
        "all three names retained as evidence"
    );
    assert_eq!(
        current_name(&c, p).await.as_deref(),
        Some("Mary Jones"),
        "legal name is the display winner"
    );
...
async fn recency_first_within_legal_diverges_from_dob() {
    ...
    assert_eq!(
        name_count(&c, p).await,
        2,
        "the old name is retained, not overwritten"
    );
}
...
async fn legal_name_takes_over_from_a_newer_alias() {
    ...
    submit_field(
        ...
        Some(&render_name_twin("MJ", Some("alias"), "patient-stated")),
    )
    .await
    .unwrap();
    assert_eq!(
...
async fn legal_tier_is_case_insensitive() {
    ...
    assert_eq!(
        use_raw, "Legal",
        "the authored `use` casing is preserved verbatim in use_raw"
    );
}
...
async fn set_union_reassertion_is_idempotent() {
    ...
    assert_eq!(
        name_count(&c, p).await,
        1,
        "re-assertion of the same name dedupes to one member"
    );
...
async fn cross_field_isolation_with_dob() {
    ...
    submit_field(
        ...
        Some("Date of birth (patient-stated): 1980 (year)"),
    )
    .await
    .unwrap();
    ...
    assert_eq!(
        demos, 1,
        "only the dob event projects into patient_demographic"
    );
}
