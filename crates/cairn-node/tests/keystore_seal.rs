...
async fn sealed_init_produces_dual_recipient_key_and_surfaces_escrow() {
    ...
    identity::provision(&db, &sk, &kid, "A", "127.0.0.1:7900")
        .await
        .unwrap();
    ...
    assert!(
        st.initialized,
        "provisioned node must report initialized=true"
    );
    assert!(st.keystore_ok);
    assert!(
        st.key_at_rest.contains("SEALED"),
        "got {:?}",
        st.key_at_rest
    );
    assert!(st.recovery_escrow, "sealed key must report an escrow");
    assert!(
        st.dr_escrow.contains("recovery code set"),
        "got {:?}",
        st.dr_escrow
    );
}
