...
async fn authored_twin_on_note_passes_through_verbatim_and_reads_authored() {
    ...
    submit_note(
        ...
        p,
        1,
        Some("Progress note: BP 120/80, afebrile"),
    )
    .await
    .expect("authored-twin note accepted");
    ...
    let twin: String = c
        ...
        .get(0);
    assert_eq!(
        twin, "Progress note: BP 120/80, afebrile",
...
async fn twinless_note_degrades_to_flagged_skeleton() {
    ...
    let twin: String = c
        ...
        .get(0);
    assert!(
        twin.starts_with("[note.added]"),
        "derived skeleton twin, got: {twin}"
    );
    assert!(
        twin.contains("BP 120/80"),
        "skeleton renders the payload (db/005 TODO closed)"
    );
    let authored: bool = c
        .query_one(
            "SELECT cairn_twin_is_authored(signed_bytes) FROM event_log WHERE patient_id::text=$1",
    ...
    assert!(
        !authored,
        "a derived twin reads back as NOT authored (honest flag)"
    );
}
...
async fn twinless_demographic_is_still_hard_rejected() {
    ...
    let err = c
        ...
        .await
        .expect_err("twin-less demographic must be rejected");
    ...
    assert!(
        msg.contains("authored twin") || msg.contains("§4.5"),
        "rejection cites the twin: {msg}"
    );
...
async fn whitespace_twin_demographic_is_still_hard_rejected() {
    ...
    let err = c
        .execute("SELECT submit_event($1)", &[&signed.signed_bytes])
        .await
        .expect_err("whitespace-only twin demographic must be rejected");
    let msg = err
        .as_db_error()
        .map(|e| e.message())
        .unwrap_or("<no db message>");
