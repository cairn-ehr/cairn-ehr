...
//! - **Test 17a — a legacy CAIRNB1/B2 medium is a NAMED outcome.** It has no clinical plane at
//!   all, and the summary must say so rather than print the CAIRNB3 "carries NO clinical records"
//!   line, whose remedy ("check the capture that wrote it") is wrong for a medium that predates
//!   the plane.
...
use cairn_medium::{Plane, SelfMarker};
...
mod restore_kit;
use restore_kit::{
    a_recovery_code, an_op_passphrase, author_sealed_clinical_event, capture, chained_segment, cs,
    in_event_log, in_pen, provisioned_clinic, restore_cli, rewrite_medium, sealed_assert_body,
...
///
/// Mutation this kills: `plane_records_with_accounting` taking every segment instead of the
/// prefix through `verified_through` — B and C are then admitted.
#[tokio::test]
async fn a_restore_applies_the_verified_prefix_and_not_one_record_past_a_chain_break() {
    ...
    let cc = author_sealed_clinical_event(&c, &sk, &kid).await;
    capture(&c, &sk, &kid, dir.path()).await;

    // Break the link of the segment holding B, and count what sits past it — by hand, from the
    // segments, never from the code under test.
    let mut past_the_break = 0;
    let mut on_the_plane = 0;
    rewrite_medium(&medium, |segments| {
        let broken = segments
            .iter()
            .position(|s| s.records.iter().any(|r| r.signed_bytes == b.signed_bytes))
            .expect("the second capture wrote B's segment");
        segments[broken].prev_commitment = "deadbeef".into();
        for (position, s) in segments.iter().enumerate() {
            if s.plane == Plane::Clinical {
                on_the_plane += s.records.len();
                if position >= broken {
                    past_the_break += s.records.len();
                }
            }
        }
    });
    assert!(
        past_the_break >= 4,
        "anti-vacuity: B's and C's segments (a registration and a chart each) must both sit past \
         the break, or 'not one record past it' is a statement about one segment"
    );
    ...
    for (label, chart) in [("B, whose link is broken", &b), ("C, hanging from B", &cc)] {
        assert!(
            !in_event_log(&c, &chart.event_id).await,
            "chart {label} sits past the last verified chain link and must not be applied"
        );
        assert!(
            !in_pen(&c, &chart.signed_bytes).await,
            "chart {label} must not be penned either — a pen row promises a requeue can finish it"
        );
    }
...
async fn a_plane_this_build_cannot_route_is_noted_with_its_count_and_never_applied() {
    ...
    rewrite_medium(&medium, |segments| {
        let records = unroutable
            .iter()
            .enumerate()
            .map(|(n, (_, bytes))| cairn_medium::MediumRecord {
                signed_bytes: bytes.clone(),
                attestation: None,
                attester_key: None,
                dek_wrapped: None,
                source_seq: 10_000 + n as i64,
            })
            .collect();
