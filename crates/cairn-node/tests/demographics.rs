...
async fn happy_path_appends_and_projects_with_authored_twin() {
    ...
    assert_identifier(&c, &sk, &kid, p, 1, &a)
        .await
        .expect("valid assertion accepted");
...
async fn set_union_dedups_same_key_keeps_different_key() {
    ...
    let same_formatted = IdentifierAssertion {
        value: "9434765919",
        ..same_clone(&same)
    };
    let other = IdentifierAssertion {
        ...
        system: "nhs-number",
        provenance: "patient-stated",
        normalized: Some("1112223334"),
        profile: Some("nhs-number@b3-abc"),
        use_: None,
    };
    assert_identifier(&c, &sk, &kid, p, 1, &same).await.unwrap();
    assert_identifier(&c, &sk, &kid, p, 2, &same_formatted)
        .await
        .unwrap(); // same normalized → dedup
    assert_identifier(&c, &sk, &kid, p, 3, &other)
        .await
        .unwrap(); // different normalized → 2nd row
    let p_str = p.to_string();
    ...
    assert_eq!(
        n, 2,
        "same-normalized dedups; different-normalized keeps both"
    );
}
...
async fn honest_degradation_no_normalized_no_profile_accepted() {
    ...
    assert_identifier(&c, &sk, &kid, p, 1, &a)
        .await
        .expect("profile-less assertion accepted");
    let p_str = p.to_string();
    let mk: String = c
        .query_one(
            "SELECT match_key FROM patient_identifier WHERE patient_id::text=$1",
            &[&p_str],
        )
        .await
        .unwrap()
    ...
    assert_eq!(
        mk, "OLD-CARD-77",
        "match_key falls back to value when normalized absent"
    );
}
...
async fn identifier_same_key_winner_is_hlc_latest_regardless_of_apply_order() {
    ...
    assert_identifier(&c, &sk, &kid, p1, 1, &earlier)
        .await
        .unwrap();
    assert_identifier(&c, &sk, &kid, p1, 2, &later)
        .await
        .unwrap();
    ...
    assert_identifier(&c, &sk, &kid, p2, 2, &later)
        .await
        .unwrap();
    assert_identifier(&c, &sk, &kid, p2, 1, &earlier)
        .await
        .unwrap();
    ...
    for (p, label) in [(p1, "forward order"), (p2, "reverse order")] {
        ...
        assert_eq!(
            value, "9434765919",
            "{label}: HLC-latest value wins deterministically"
        );
    }
}
