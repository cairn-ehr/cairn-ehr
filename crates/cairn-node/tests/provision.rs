...
async fn provision_writes_genesis_identity() {
    ...
    let node_id = identity::provision(&client, &sk, &kid, "Clinic-A", "127.0.0.1:7800")
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.fingerprint,
        cairn_event::short_fingerprint(&kid).unwrap()
    );

    // Genesis is once-only: a second provision must error.
    assert!(
        identity::provision(&client, &sk, &kid, "Clinic-A", "127.0.0.1:7800")
            .await
            .is_err()
    );
}
