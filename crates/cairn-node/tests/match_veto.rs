...
async fn submit_identifier(
    ...
    submit(
        ...
        identifier_assertion_body(a),
        Some(&render_identifier_twin(a)),
    )
    .await
...
) -> IdentifierAssertion<'a> {
    IdentifierAssertion {
        value,
        system,
        provenance: "patient-stated",
        normalized,
        profile: if normalized.is_some() {
            Some("test-profile@stub")
        } else {
            None
        },
        use_: None,
...
async fn no_veto_when_no_shared_system() {
    ...
    submit_identifier(
        ...
        a,
        1,
        &idassert("medicare-au", "1111", Some("1111")),
    )
    .await;
    submit_identifier(
        &c,
        ...
        b,
        2,
        &idassert("nhs-number", "2222", Some("2222")),
    )
    .await;
    assert!(
        veto_rows(&c, a, b).await.is_empty(),
        "disjoint systems raise no veto"
    );
...
async fn identifier_hard_veto_when_normalized_present_and_disjoint() {
    ...
    submit_identifier(
        ...
        a,
        1,
        &idassert("medicare-au", "9434765919", Some("9434765919")),
    )
    .await;
    submit_identifier(
        &c,
        ...
        b,
        2,
        &idassert("medicare-au", "5000000000", Some("5000000000")),
    )
    .await;
...
async fn identifier_same_normalized_is_no_veto() {
    ...
    submit_identifier(
        ...
        b,
        2,
        &idassert("medicare-au", "943 476 5919", Some("9434765919")),
    )
    .await;
    assert!(
        veto_rows(&c, a, b).await.is_empty(),
        "shared normalized = positive evidence"
    );
}
...
async fn identifier_degrade_hold_when_profile_absent_and_values_differ() {
    ...
    assert!(
        !has_hard_veto(&c, a, b).await,
        "degrade_hold does not trip the auto-link gate"
    );
}
...
async fn identifier_degrade_hold_when_one_side_normalized_other_not() {
    ...
    submit_identifier(
        ...
        a,
        1,
        &idassert("local-mrn", "00123", Some("00123")),
    )
    .await;
    ...
    assert!(
        !has_hard_veto(&c, a, b).await,
        "asymmetric-normalized mismatch must not be a hard veto"
    );
}
...
async fn unknown_system_never_vetoes() {
    ...
    submit_identifier(
        ...
        b,
        2,
        &idassert("unknown", "BBB", Some("BBB")),
    )
    .await;
    assert!(
        veto_rows(&c, a, b).await.is_empty(),
        "system 'unknown' never participates in a veto"
    );
}
...
async fn multi_valued_shared_value_is_no_veto() {
    ...
    submit_identifier(
        ...
        a,
        1,
        &idassert("medicare-au", "1000000000", Some("1000000000")),
    )
    .await;
    submit_identifier(
        &c,
        ...
        a,
        2,
        &idassert("medicare-au", "2000000000", Some("2000000000")),
    )
    .await;
    submit_identifier(
        &c,
    ...
    assert!(
        veto_rows(&c, a, b).await.is_empty(),
        "one shared normalized in the set = no veto"
    );
}
...
async fn submit_sex(
    ...
    .await
    .expect("valid sex-at-birth accepted");
}
...
async fn dob_no_veto_when_precision_differs() {
    ...
    assert!(
        veto_rows(&c, a, b).await.is_empty(),
        "different precision = no finding"
    );
}
...
async fn dob_no_veto_when_not_both_verified() {
    ...
    assert!(
        veto_rows(&c, a, b).await.is_empty(),
        "clash only on verified-vs-verified"
    );
}
...
async fn veto_is_symmetric() {
    ...
    .await;
    assert_eq!(
        veto_rows_full(&c, a, b).await,
        veto_rows_full(&c, b, a).await,
