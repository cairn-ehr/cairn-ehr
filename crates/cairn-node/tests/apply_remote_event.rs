...
async fn setup(c: &Client) -> (SigningKey, String, SigningKey, String) {
    ...
    c.execute("SELECT enroll_actor('human', '{\"role\":\"clinician\"}', $1)", &[&kid_h])
        .await.unwrap();
...
async fn malformed_bytes_are_refused() {
    ...
    let m = db_msg(&err);
    assert!(m.contains("signature") || m.contains("verif"), "legible verify reject: {m}");
}
...
async fn unenrolled_signer_is_refused_at_apply() {
    ...
    assert!(db_msg(&err).contains("enrolled"), "cites enrollment: {}", db_msg(&err));
    assert_eq!(event_count(&c, p).await, 0, "refused event must not be stored");
}
...
async fn unknown_event_type_is_refused_fail_closed() {
    ...
    let err = apply(&c, &signed.signed_bytes).await.unwrap_err();
    assert!(db_msg(&err).contains("fail closed"), "unknown type fails closed: {}", db_msg(&err));
}
...
async fn enrolled_signer_event_applies_projects_and_merges_hlc() {
    ...
    let wall: i64 = c.query_one("SELECT hlc_wall FROM hlc_state", &[]).await.unwrap().get(0);
    assert!(wall >= WALL_2026, "hlc_state merged forward: {wall}");

    // Idempotent re-apply is a silent no-op (set-union), never an error.
    apply(&c, &signed.signed_bytes).await.expect("re-apply of the same bytes is idempotent");
...
async fn event_id_substitution_is_refused_at_apply() {
    ...
    apply(&c, &sign(&b1, &sk).unwrap().signed_bytes).await.unwrap();
    let err = apply(&c, &sign(&b2, &sk).unwrap().signed_bytes).await.unwrap_err();
    assert!(db_msg(&err).contains("substitution"), "substitution refused: {}", db_msg(&err));
}
...
async fn unattested_suppress_is_refused_at_apply() {
    ...
    apply(&c, &sign(&target, &sk).unwrap().signed_bytes).await.unwrap();
    ...
    let err = apply(&c, &signed.signed_bytes).await.unwrap_err();
    assert!(db_msg(&err).contains("attestation"), "un-attested suppress refused: {}", db_msg(&err));
...
async fn attested_suppress_applies_and_stores_the_token() {
    ...
    ).await.expect("attested suppress applies");
    ...
    assert_eq!(att.as_deref(), Some(&token[..]), "attestation token stored verbatim");
...
async fn suppress_targeting_unknown_event_is_refused() {
    ...
    let err = c.execute(
        "SELECT apply_remote_event($1, $2, $3)",
        &[&signed.signed_bytes, &token, &hkey],
    ).await.expect_err("orphan suppress refused (retried once the target arrives)");
    assert!(db_msg(&err).contains("unknown event"), "cites the target: {}", db_msg(&err));
}
...
async fn twinless_demographic_is_refused_at_apply_like_submit() {
    ...
    let body = EventBody {
        ...
        hlc: Hlc { wall: WALL_2026, counter: 0, node_origin: "peer".into() },
        t_effective: None,
    ...
    let err = apply(&c, &signed.signed_bytes).await.unwrap_err();
    let m = db_msg(&err);
    assert!(m.contains("authored twin") || m.contains("§4.5"), "cites the twin floor: {m}");
...
async fn twinless_note_derives_the_same_skeleton_at_both_doors() {
    ...
    apply(&c, &sign(&b1, &sk).unwrap().signed_bytes).await.unwrap();
    c.execute("SELECT submit_event($1)", &[&sign(&b2, &sk).unwrap().signed_bytes]).await.unwrap();
    let rows = c.query(
    ...
    assert_eq!(t1, t2, "both doors derive via the one in-DB skeleton renderer");
}
...
async fn offsetless_t_effective_is_refused_at_both_doors() {
    ...
    let signed = sign(&b, &sk).unwrap();
    let err = apply(&c, &signed.signed_bytes).await.unwrap_err();
    assert!(db_msg(&err).contains("offset"), "apply door cites the offset pin: {}", db_msg(&err));
    let err = c.execute("SELECT submit_event($1)", &[&signed.signed_bytes]).await.unwrap_err();
    assert!(db_msg(&err).contains("offset"), "authoring door too: {}", db_msg(&err));
    assert_eq!(event_count(&c, p).await, 0);
}
...
async fn explicit_offset_t_effective_is_stored_timezone_independently() {
    ...
    apply(&c, &sign(&b, &sk).unwrap().signed_bytes).await.expect("explicit offset accepted");
    let same_instant: bool = c.query_one(
        "SELECT t_effective = '2026-06-20T08:00:00Z'::timestamptz \
         FROM event_log WHERE patient_id = $1::text::uuid",
        &[&p.to_string()],
    ).await.unwrap().get(0);
    assert!(same_instant, "stored t_effective is the offset-corrected UTC instant");
    c.batch_execute("RESET TimeZone; RESET DateStyle").await.unwrap();
}
...
async fn forward_dated_t_effective_is_refused_at_apply() {
    ...
    let err = apply(&c, &sign(&b, &sk).unwrap().signed_bytes).await.unwrap_err();
    assert!(db_msg(&err).contains("t_recorded"), "cites the ceiling: {}", db_msg(&err));
}
...
fn link_event(kid: &str, a: Uuid, b: Uuid, wall: i64) -> EventBody {
    ...
    let la = LinkAssertion {
        subject_a: &a_s, subject_b: &b_s, provenance: "matcher:cfg@test", confidence: None,
    };
    EventBody {
        event_id: Uuid::now_v7().to_string(),
...
async fn oversize_component_clamps_and_flags_at_apply_never_vetoes() {
    ...
    let c = db::connect_and_load_schema(&base).await.unwrap();
    let (sk, kid, _, _) = setup(&c).await;
    c.batch_execute("SET cairn.max_component_size = 2").await.unwrap();
    let (a, b, cc) = (Uuid::now_v7(), Uuid::now_v7(), Uuid::now_v7());

    // {A,B} = 2 ≤ cap: applies and projects normally.
    let l1 = link_event(&kid, a, b, WALL_2026);
    apply(&c, &sign(&l1, &sk).unwrap().signed_bytes).await.expect("at-cap link applies");
    let n: i64 = c.query_one("SELECT count(*) FROM person_member", &[]).await.unwrap().get(0);
    ...
    apply(&c, &sign(&l2, &sk).unwrap().signed_bytes)
        .await.expect("oversize component must not veto a replicated event (A5b)");
    let stored: i64 = c.query_one(
    ...
    let flags: i64 = c.query_one(
        "SELECT count(*) FROM identity_projection_flag WHERE observed_size > cap", &[],
    ).await.unwrap().get(0);
    assert!(flags >= 1, "the skipped recompute left a loud worklist flag");
    ...
    c.execute("SELECT submit_event($1)", &[&sign(&l3, &sk).unwrap().signed_bytes])
        .await.expect("fresh 2-member link fine locally");
    let l4 = link_event(&kid, e_, cc, WALL_2026 + 30); // joins {D,E} to {A,B,C} → 5 > 2
    let err = c.execute("SELECT submit_event($1)", &[&sign(&l4, &sk).unwrap().signed_bytes])
        .await.unwrap_err();
    assert!(db_msg(&err).contains("exceeds max size"),
        "local authoring still vetoes fail-loud: {}", db_msg(&err));
    c.batch_execute("RESET cairn.max_component_size").await.unwrap();
}
...
async fn oversized_event_is_refused_at_apply() {
    ...
    let signed = sign(&b, &sk).unwrap();
    let err = apply(&c, &signed.signed_bytes).await.unwrap_err();
    assert!(db_msg(&err).contains("ceiling"), "cites the admission ceiling: {}", db_msg(&err));
}
