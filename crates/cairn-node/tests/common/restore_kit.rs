...
//!
//! `restore_cli_surface.rs` (#572/#570) built these fixtures privately. Slice 2d's remaining
//! design tests (#593) need the same dead clinic, the same export and the same wiped replacement
//! machine from four more suites, and every one of these helpers has already had a way to make a
//! test pass for the wrong reason — the wipe that forgot `actor_event`, the export that was not
//! really beside the medium. A fixture with that history should exist ONCE, which is the
//! convention `clinic_kit.rs` set for #567.
//!
...
//! 5. [`restore_cli`] — the real `cairn-node restore`, with piped streams and no terminal.
//!
...
/// **Pure** apart from the fresh DEK and medication id `seal_event_payload` mints.
///
...
/// Write the sealed `CAIRNL1` export sibling beside `medium`, exactly as the `backup` command
/// does after its capture.
///
...
///
/// ⚠️ **The registry is the half that is easy to forget, and forgetting it makes a restore test
/// weaker than it looks.** `actor_event` survives a clinical-tier truncate, so a wipe that
...
/// arrived. A real replacement machine has an empty `actor_event`, which is exactly why
/// `restore_actor_registry` (db/052) exists at all.
///
...
/// Run the real `cairn-node restore` against `conn`, with piped streams and NO terminal.
///
/// `--insecure-plaintext` keeps the new key unsealed, so no passphrase or freshly minted
/// recovery code is involved; `code_file` is the DEAD node's recovery code, which opens the
/// export beside the medium (ADR-0069). `None` means the export is never opened — or there is
/// none to open.
...
/// Parse the CAIRNB3 medium at `path`, let `edit` change its segments, and write it back.
///
...
/// is modelling.
pub fn rewrite_medium(path: &Path, edit: impl FnOnce(&mut Vec<Segment>)) {
