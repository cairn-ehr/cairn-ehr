...
use cairn_event::identity::{
    repudiation_assertion_body, render_repudiate_twin, RepudiationAssertion,
};
...
fn repudiation_body(
    kid_a: &str, subject: Uuid, value: &str, reason: &str, twin: bool,
) -> EventBody {
    let s_s = subject.to_string();
    let a = RepudiationAssertion { subject: &s_s, value, reason };
    EventBody {
        event_id: Uuid::now_v7().to_string(),
        ...
        attachments: vec![],
        plaintext_twin: if twin { Some(render_repudiate_twin(&a)) } else { None },
    }
}
...
#[allow(clippy::too_many_arguments)]
async fn repudiate(
    c: &Client, sk_a: &SigningKey, kid_a: &str, sk_h: &SigningKey, kid_h: &str,
    subject: Uuid, value: &str, reason: &str, wall: i64,
    ...
    c.execute("SELECT submit_event($1,$2,$3)", &[&signed.signed_bytes, &token, &vk_h]).await
}

async fn current_name(c: &Client, p: Uuid) -> Option<String> {
    let p_str = p.to_string();
    c.query_opt("SELECT value FROM patient_name_current WHERE patient_id::text=$1", &[&p_str])
        .await.unwrap().map(|r| r.get(0))
}

async fn name_count(c: &Client, p: Uuid) -> i64 {
    let p_str = p.to_string();
    c.query_one("SELECT count(*) FROM patient_name WHERE patient_id::text=$1", &[&p_str])
        .await.unwrap().get(0)
}
...
async fn repudiated_name_leaves_the_winner_but_a_surviving_name_takes_over() {
    ...
    submit_name(&c, &sk_a, &kid_a, p, 2, "John Smith", "legal", "patient-stated").await;
    submit_name(&c, &sk_a, &kid_a, p, 1, "Real Alias", "alias", "patient-stated").await;
    assert_eq!(current_name(&c, p).await.as_deref(), Some("John Smith"),
               "the fabricated legal name is the winner before repudiation");

    repudiate(&c, &sk_a, &kid_a, &sk_h, &kid_h, p, "John Smith",
              "confessed fabricated persona", 3).await
        .expect("human-attested repudiation accepted");

    assert_eq!(current_name(&c, p).await.as_deref(), Some("Real Alias"),
...
async fn repudiating_the_only_name_yields_no_winner_not_a_lie() {
    ...
    submit_name(&c, &sk_a, &kid_a, p, 1, "Jane Doe", "legal", "patient-stated").await;
    assert_eq!(current_name(&c, p).await.as_deref(), Some("Jane Doe"));
    repudiate(&c, &sk_a, &kid_a, &sk_h, &kid_h, p, "Jane Doe", "fabricated", 2).await.unwrap();
    assert_eq!(current_name(&c, p).await, None,
               "no surviving name → no winner (honest), never the struck name");
    assert_eq!(name_count(&c, p).await, 1, "the struck name is retained as evidence");
}
...
async fn reassert_is_idempotent_and_reason_is_hlc_latest_wins() {
    ...
    repudiate(&c, &sk_a, &kid_a, &sk_h, &kid_h, p, "Al Ias", "first reason", 10).await.unwrap();
    repudiate(&c, &sk_a, &kid_a, &sk_h, &kid_h, p, "Al Ias", "refined reason", 20).await.unwrap();
    // An OLDER re-assert must not overwrite the newer standing reason.
    repudiate(&c, &sk_a, &kid_a, &sk_h, &kid_h, p, "Al Ias", "stale reason", 5).await.unwrap();
...
async fn unattested_repudiation_is_refused() {
    ...
    let p = Uuid::now_v7();
    submit_name(&c, &sk_a, &kid_a, p, 1, "No Token", "legal", "patient-stated").await;
    ...
    assert!(db_msg(&err).contains("requires attestation"),
            "an un-attested suppressing repudiation must be refused: {}", db_msg(&err));
...
async fn bad_subject_is_rejected() {
    ...
    let token = sign_attestation(&ca, &kid_h, "attested", &sk_h).unwrap();
    let vk_h = sk_h.verifying_key().to_bytes().to_vec();
    let err = c.execute("SELECT submit_event($1,$2,$3)", &[&signed.signed_bytes, &token, &vk_h])
        .await.unwrap_err();
...
async fn missing_twin_is_rejected() {
    ...
    assert!(db_msg(&err).contains("authored twin"),
            "a twin-less repudiation must be refused: {}", db_msg(&err));
}
