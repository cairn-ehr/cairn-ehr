...
DECLARE
    ...
    v_identity_state boolean := false;
BEGIN
    ...
    -- Absent/blank twin: demographic, identity-link, dispute, AND identity-state types
    -- HARD-require it; every other type degrades honestly to a flagged derived skeleton
    -- (ADR-0039).
    ...
    ELSIF v_identity_state THEN
        RAISE EXCEPTION 'submit_event: identity-state assertion requires a non-empty authored twin (§5.7)';
    END IF;
